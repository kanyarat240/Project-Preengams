...
namespace Line.Messaging
{
    ...
    public class LineMessagingClient : ILineMessagingClient, IDisposable
    {
        ...
        public LineMessagingClient(string channelAccessToken, string baseURL, string clientId)
        {
            ...
            _clientid = clientId;
            //_noti = noti;,ILineHttpNotify noti
        ...
        public static async Task<ChannelAccessToken> IssueChannelAccessTokenAsync(HttpClient httpClient, string client_assertion)
        {
            var response = await httpClient.PostAsync($"oauth2/v2.1/token",
            ...
            ChannelAccessToken result = JsonConvert.DeserializeObject<ChannelAccessToken>(json,
            ...
            result.ExpirationIn = DateTime.Now.AddSeconds(result.ExpiresIn - 60);
            return result;
        }
        ...
        /// <param name="channelAccessToken">ChannelAccessToken</param>
        /// <param name="uri">URI</param>
        public static async Task RevokeChannelAccessTokenAsync(HttpClient httpClient, string channelAccessToken, string clientId)
        {
            var response = await httpClient.PostAsync($"/oauth2/v2.1/revoke",
                new FormUrlEncodedContent(new Dictionary<string, string>
                {
                    ["client_id"] = clientId,
        ...
        /// <param name="uri">URI</param>
        /// <returns></returns>
        public static async Task<LineMessagingClient> CreateAsync(string baseURL, string channelId, string longliveToken)
        {
            //, dynamic privateKey
            //if (string.IsNullOrEmpty(channelId)) { throw new ArgumentNullException(nameof(channelId)); }
            //if (privateKey == null) { throw new ArgumentNullException(nameof(privateKey)); }
            //, ILineHttpNotify noti
            using (var client = new HttpClient() { BaseAddress = new Uri(baseURL) })
            {
                //string privateKeyJson = JsonConvert.SerializeObject(privateKey, Formatting.None);

                //int token_exp = 60 * 60 * 24 * 30;

                //JsonWebKey jsonWeb = JsonWebKey.Create(privateKeyJson);
                ////jsonWeb.KeyId = kid;

                //var signingCredentials = new SigningCredentials(jsonWeb, jsonWeb.Alg);
                ...
                //    new Claim(JwtRegisteredClaimNames.Sub,channelId),
                //    new Claim("token_exp",token_exp.ToString(), ClaimValueTypes.Integer32)
                //}),
                //    Expires = now.AddMinutes(29),
                //    SigningCredentials = signingCredentials
                //};
                //var client_assertion = tokenHandler.CreateToken(tokenDescriptor);

                //ChannelAccessToken channelAccessToken = null;
                //string strAccessToken = Environment.GetEnvironmentVariable("LINE_API_ACCESS_TOKEN");
                //if (string.IsNullOrWhiteSpace(strAccessToken))
                //{
                //    channelAccessToken = await IssueChannelAccessTokenAsync(client, tokenHandler.WriteToken(client_assertion));
                //}
                //else
                //{
                //    channelAccessToken = JsonConvert.DeserializeObject<ChannelAccessToken>(strAccessToken);
                //    if (channelAccessToken.ExpirationIn <= DateTime.Now)
                //    {
                //        //Refesh Token
                //    }
                //}

                return new LineMessagingClient(longliveToken, baseURL, channelId);
            }
        }
        ...
        {
            string json = JsonConvert.SerializeObject(new { replyToken, messages }, _jsonSerializerSettings);
            StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
            //_noti.Notify(new HttpRequestLogDTO() { Url = "/v2/bot/message/reply", Json = json, HttpMethod = "POST" });

            var response = await _client.PostAsync($"/v2/bot/message/reply", content).ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        ...
        public virtual async Task ReplyMessageWithJsonAsync(string replyToken, string messages)
        {
            var json = $@"{{ 
        ...
        {
            var _content = JsonConvert.SerializeObject(new { to, messages }, _jsonSerializerSettings);
            StringContent content = new StringContent(_content, Encoding.UTF8, "application/json");

            //_noti.Notify(new HttpRequestLogDTO() { Url = "/v2/bot/message/push", Json = _content, HttpMethod = "POST" });

            var response = await _client.PostAsync($"/v2/bot/message/push", content).ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        /// <summary>
        ...
        public virtual async Task PushMessageWithJsonAsync(string to, params string[] messages)
        {
            ...
            StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
            //_noti.Notify(new HttpRequestLogDTO() { Url = "/v2/bot/message/push", Json = json, HttpMethod = "POST" });
            var response = await _client.PostAsync($"/v2/bot/message/push", content).ConfigureAwait(false);
        ...
        public virtual async Task MultiCastMessageAsync(IList<string> to, IList<ISendMessage> messages)
        {
            ...
            //_noti.Notify(new HttpRequestLogDTO() { Url = "/v2/bot/message/multicast", Json = content, HttpMethod = "POST" });

            var response = await _client.PostAsync($"/v2/bot/message/multicast", new StringContent(content, Encoding.UTF8, "application/json")).ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        /// <summary>
        ...
        public virtual async Task MultiCastMessageWithJsonAsync(IList<string> to, params string[] messages)
        {
            ...
            //_noti.Notify(new HttpRequestLogDTO() { Url = "/v2/bot/message/multicast", Json = json,HttpMethod="POST" });

            var response = await _client.PostAsync($"/v2/bot/message/multicast", new StringContent(json, Encoding.UTF8, "application/json")).ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/message/{messageId}/content",HttpMethod="GET" });
            var response = await _client.GetAsync($"/v2/bot/message/{messageId}/content").ConfigureAwait(false);
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/profile/{userId}" ,HttpMethod="GET"});
            var content = await GetStringAsync($"/v2/bot/profile/{userId}").ConfigureAwait(false);
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/group/{groupId}/member/{userId}" ,HttpMethod="GET"});
            var content = await GetStringAsync($"/v2/bot/group/{groupId}/member/{userId}").ConfigureAwait(false);
        ...
        {
            var requestUrl = $"/v2/bot/group/{groupId}/members/ids";
            if (continuationToken != null)
            {
                requestUrl += $"?start={continuationToken}";
            }
            //_noti.Notify(new HttpRequestLogDTO() { Url = requestUrl,HttpMethod="GET" });
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/group/{groupId}/leave" ,HttpMethod="POST"});
            var response = await _client.PostAsync($"/v2/bot/group/{groupId}/leave", null).ConfigureAwait(false);
        ...
        public virtual async Task<UserProfile> GetRoomMemberProfileAsync(string roomId, string userId)
        {
            ...
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/bot/room/{roomId}/member/{userId}", HttpMethod = "GET" });
            var content = await GetStringAsync($"/bot/room/{roomId}/member/{userId}").ConfigureAwait(false);
        ...
        {
            var requestUrl = $"/v2/bot/room/{roomId}/members/ids";
            if (continuationToken != null)
            {
                requestUrl += $"?start={continuationToken}";
            }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/room/{roomId}/leave", HttpMethod = "GET" });
            var response = await _client.PostAsync($"/v2/bot/room/{roomId}/leave", null).ConfigureAwait(false);
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/richmenu/{richMenuId}", HttpMethod = "GET" });
            var json = await GetStringAsync($"/v2/bot/richmenu/{richMenuId}").ConfigureAwait(false);
        ...
        public virtual async Task<string> CreateRichMenuAsync(RichMenu richMenu)
        {
            ...
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/richmenu",Json= content, HttpMethod = "POST" });
            var response = await _client.PostAsync($"/v2/bot/richmenu", new StringContent(content, Encoding.UTF8, "application/json")).ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/richmenu/{richMenuId}", HttpMethod="DELETE"});
            var response = await _client.DeleteAsync($"/v2/bot/richmenu/{richMenuId}");
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/user/{userId}/richmenu", HttpMethod = "GET" });
            var json = await GetStringAsync($"/v2/bot/user/{userId}/richmenu");
            return JsonConvert.DeserializeAnonymousType(json, new { richMenuId = "" }).richMenuId;
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/user/all/richmenu/{richMenuId}", HttpMethod = "POST" });
            var response = await _client.PostAsync($"/v2/bot/user/all/richmenu/{richMenuId}", null);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/user/{userId}/richmenu/{richMenuId}", HttpMethod = "POST" });
            var response = await _client.PostAsync($"/v2/bot/user/{userId}/richmenu/{richMenuId}", null);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/user/{userId}/richmenu", HttpMethod = "DELETE" });
            var response = await _client.DeleteAsync($"/v2/bot/user/{userId}/richmenu").ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/richmenu/{richMenuId}/content", HttpMethod = "GET" });
            var response = await _client.GetAsync($"/v2/bot/richmenu/{richMenuId}/content").ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
            return new ContentStream(await response.Content.ReadAsStreamAsync(), response.Content.Headers);
        }
        ...
        protected async Task UploadRichMenuImageAsync(Stream stream, string richMenuId, string mediaType)
        {
            ...
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/richmenu/{richMenuId}/content", HttpMethod = "POST" });
            var response = await _client.PostAsync($"/v2/bot/richmenu/{richMenuId}/content", Content).ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/richmenu/list", HttpMethod = "GET" });
            var response = await _client.GetAsync($"/v2/bot/richmenu/list").ConfigureAwait(false);
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/user/{userId}/linkToken", HttpMethod = "POST" });
            var response = await _client.PostAsync($"/v2/bot/user/{userId}/linkToken", null);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/message/delivery/reply?date={date.ToString("yyyyMMdd")}", HttpMethod = "GET" });
            var response = await GetStringAsync($"/v2/bot/message/delivery/reply?date={date.ToString("yyyyMMdd")}");
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/message/delivery/push?date={date.ToString("yyyyMMdd")}", HttpMethod = "GET" });
            var response = await GetStringAsync($"/v2/bot/message/delivery/push?date={date.ToString("yyyyMMdd")}");
            return JsonConvert.DeserializeObject<NumberOfSentMessages>(response);
        }
        ...
        {
            //_noti.Notify(new HttpRequestLogDTO() { Url = $"/v2/bot/message/delivery/multicast?date={date.ToString("yyyyMMdd")}", HttpMethod = "GET" });
            var response = await GetStringAsync($"/v2/bot/message/delivery/multicast?date={date.ToString("yyyyMMdd")}");
            return JsonConvert.DeserializeObject<NumberOfSentMessages>(response);
        }
        ...
        public async Task<GroupSummary> GetGroupSummaryAsync(string groupId)
        {
            var response = await GetStringAsync($"/v2/bot/group/{groupId}/summary");
            return JsonConvert.DeserializeObject<GroupSummary>(response);
        }
    }
}
